...
/*    
	...
	dim3 dimBlock(TileSize,TileSize);
        ...
        update_levelset_data_cuda<<<dimGrid,dimBlock>>>(devicex, devicey, dxadv, dyadv, dcellx, dcelly, dtracker, t, dt,
                dphi, dpsix, dpsiy, dpsixy, masterdphi, masterdpsix, masterdpsiy,psischeme,backtrace_scheme,T_period,nx,ny,TileSize);
        devicetodevicecopy(dphi,dpsix,dpsiy,masterdphi,masterdpsix,masterdpsiy,nx,TileSize); 
	// Update the mixed derivatives now for the remaining grid points
        update_mixed_derivatives(dpsix, dpsiy, dpsixy, nx, ny, dx, dy,TileSize);
